...
namespace ISAAR.MSolve.Analyzers.Multiscale
{
    ...
    /// </summary>
    public class MicrostructureBvpNRNLAnalyzerDevelop : IChildAnalyzer
    {
        ...
        private Dictionary<int,Dictionary<int, IVector>> iterSolutions = new Dictionary<int,Dictionary<int, IVector>>();
        private Dictionary<int, IVector> iterSolutionGathered = new Dictionary<int, IVector>();
        ...
        public MicrostructureBvpNRNLAnalyzerDevelop(IStructuralModel model, ISolver solver, Dictionary<int, NonLinearSubdomainUpdaterWithInitialConditions> subdomainUpdaters,
        ...
        private void Storesolution(int step)
        {
            Dictionary<int, IVector> IterativeSOlution = new Dictionary<int, IVector>();

            foreach (ILinearSystem linearSystem in linearSystems.Values)
            {
                int id = linearSystem.Subdomain.ID;
                IterativeSOlution.Add(id, linearSystem.Solution.Copy());
            }

            iterSolutions.Add(step, IterativeSOlution);

            if(solver is FetiDPSolver)
            {
                var fetiSolver = solver as FetiDPSolver;
                ...
                iterSolutionGathered.Add(step, fetiSolver.GatherGlobalDisplacements(sudomainDisplacements));
            }
        ...
        public Dictionary<int, Dictionary<int, IVector>> GetLinearSystemSolutionsForEachIteration()
        {
            return iterSolutions;
        }

        public Dictionary<int, IVector> GetSolutionForEachIteration()
        {
            return iterSolutionGathered;
        }
