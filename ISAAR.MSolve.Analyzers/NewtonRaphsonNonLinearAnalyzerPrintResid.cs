...
{
    public class NewtonRaphsonNonLinearAnalyzerPrintResid : IAnalyzer
    {
        ...
        public NewtonRaphsonNonLinearAnalyzerPrintResid(ISolver solver, ILinearSystem[] linearSystems, INonLinearSubdomainUpdater[] subdomainUpdaters, ISubdomainGlobalMapping[] mappings,
        ...
        string string1 = @"C:\Users\turbo-x\Desktop\notes_elegxoi\MSOLVE_output_2\Residual_{0}.txt";
        string string2 = @"C:\Users\turbo-x\Desktop\notes_elegxoi\MSOLVE_output_2\InternalRHS_{0}.txt";
        private double CalculateInternalRHS(int currentIncrement, int step) 
        {
            ...
            foreach (ILinearSystem subdomain in linearSystems)
            {
                ...
                string path2 = string.Format(string2, counter_data);
                //solution.WriteToFile(path);
                double[] RHS_data = new double[subdomainRHS.Length];
                subdomainRHS.CopyTo(RHS_data, 0);
                WriteToFileVector(RHS_data, path);

                double[] intRHS_data = new double[internalRHS.Length];
                internalRHS.CopyTo(intRHS_data, 0);
                WriteToFileVector(intRHS_data, path2);
            }
