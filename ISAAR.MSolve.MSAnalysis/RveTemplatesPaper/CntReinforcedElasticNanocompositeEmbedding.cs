...
{
    public class CntReinforcedElasticNanocompositeEmbedding : IdegenerateRVEbuilder
    {
        int hexa1 = 10;
        int hexa2 = 10;
        int hexa3 = 10;
        ...
        public CntReinforcedElasticNanocompositeEmbedding(IIsotropicContinuumMaterial3D matrixMaterial)
        {
        ...
        private (int[] cntNodeIds, double[,] cntNodeCoordinates, int[,] cntElementConnectivity) GetCntBeamsNodesData(int hostNodes, int hostElements)
        {
            ...
            for (int i1 = 0; i1 < numberOfCnts; i1++)
            {
                var randomY = trandom.ContinuousUniform(-L02/2.0, L02/2.0);
                var randomZ = trandom.ContinuousUniform(-L03/2.0, L03/2.0);

                cntNodeIds[2 * i1] = hostNodes + 2 * i1;
                cntNodeIds[2 * i1 + 1] = hostNodes + 2 * i1 + 1;

                

                cntElementConnectivity[i1, 0] = cntNodeIds[2 * i1];
                cntElementConnectivity[i1, 1] = cntNodeIds[2 * i1 + 1];
            }

            int i = 0;
            cntNodeCoordinates[2 * i, 0] = -L01 / 2.0 + 1;
            cntNodeCoordinates[2 * i, 1] = -L02 / 2.0;
            cntNodeCoordinates[2 * i, 2] = L02 / 2.0;

            cntNodeCoordinates[2 * i + 1, 0] = L01 / 2.0 - 1;
            cntNodeCoordinates[2 * i + 1, 1] = -L02 / 2.0;
            cntNodeCoordinates[2 * i + 1, 2] = L02 / 2.0;
