...
namespace ISAAR.MSolve.Tests.FEMpartB
{
    public static class LinearRves
    {
        ...
        public static void CheckDEfGradScaleTransitions2DAndMicrostructure()
        {
            ...
            var Vec1 = Vector.CreateFromArray(new double[3] { 1, 0, 0 });
            var Vec2 = Vector.CreateFromArray(new double[3] { 0, 1, 0 });
            var strain = new double[3] { 0.0101, 0, 2*0.00510 };
            ...
            var microstructure3 = new MicrostructureDefGrad2D(homogeneousRveBuilder2,
                model => (new SkylineSolver.Builder()).BuildSolver(model), false, 1);

            microstructure3.UpdateMaterial(new double[4] { 1.01,1, 0.01,0 });
            var consCHECK = microstructure3.ConstitutiveMatrix;
