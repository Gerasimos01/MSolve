...
namespace ISAAR.MSolve.Tests.FEM
{
    ...
    //        {
    //            model.NodesDictionary.Add(nNode + 1, new Node3D( nNode + 1, nodeData[nNode, 0]+correction, nodeData[nNode, 1] + correction, nodeData[nNode, 2] + correction));
    ...
    //            List<Node3D> nodeSet = new List<Node3D>(8);
    //            for (int j = 0; j < 8; j++)
    //            {
    //                int ren1 = ContinuumHexa8NodesNumbering[j ];
    //                int nodeID = elementData[nElement, ren1];
    //                nodeSet.Add((Node3D)model.NodesDictionary[nodeID]);
    //            }
    ...
    //            {
    //                ID = nElement + 1,
    //                ElementType = factory.CreateElement(CellType3D.Hexa8, nodeSet)  // dixws to e. exoume sfalma enw sto beambuilding oxi//edw kaleitai me ena orisma to Hexa8                    
    //            };
    //            for (int j = 0; j < 8; j++)
    //            {
    //                int ren1 = ContinuumHexa8NodesNumbering[j];
