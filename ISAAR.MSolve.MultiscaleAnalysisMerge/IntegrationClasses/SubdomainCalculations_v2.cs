...
namespace ISAAR.MSolve.FEM
{
    public static class SubdomainCalculations_v2
    {
        ...
        public static double[,] CalculateDqCondDq_v2(double[][] f4_vectors, IScaleTransitions_v2 scaleTransitions, Dictionary<int, Node> boundaryNodes)
        {
        ...
        public static double[] CalculateDqFpp_v2(double[] FppReactionVector, IScaleTransitions_v2 scaleTransitions, Dictionary<int, Node> boundaryNodes)
        {
        ...
        public static double[] CalculateFppReactionsVector_v2(Subdomain_v2 subdomain, IElementMatrixProvider elementProvider,
            IScaleTransitions_v2 scaleTransitions, Dictionary<int, Node> boundaryNodes, IVectorView solution, IVectorView dSolution,
            ...
            int nIncrement, int totalIncrements)
        {
            ...
            foreach (Element element in subdomain.Elements)
            {
                ...
                var elStart = DateTime.Now;
                //IMatrix2D ElementK = elementProvider.Matrix(element);
                var localSolution = subdomain.GetLocalVectorFromGlobalWithoutPrescribedDisplacements(element, solution).ToRawArray();
                ...
                double[] localdSolution = subdomain.GetLocalVectorFromGlobalWithoutPrescribedDisplacements(element, dSolution).ToRawArray();
                double[] f = element.ElementType.CalculateForces(element, localSolution, localdSolution);
        ...
        public static double[][] CalculateKpfKffinverseKfpDq_v2(double[][] f2_vectors, Subdomain_v2 subdomain, IElementMatrixProvider elementProvider, IScaleTransitions_v2 scaleTransitions, Dictionary<int, Node> boundaryNodes)
        {
            ...
            times.Add("element", TimeSpan.Zero);
            times.Add("addition", TimeSpan.Zero);
            foreach (Element element in subdomain.Elements) //_v2.3 ElementsDictionary.Values)
            {
                var isEmbeddedElement = element.ElementType is ISAAR.MSolve.FEM.Interfaces.IEmbeddedElement;
                var elStart = DateTime.Now;
                IMatrix2D ElementK = elementProvider.Matrix(element);
