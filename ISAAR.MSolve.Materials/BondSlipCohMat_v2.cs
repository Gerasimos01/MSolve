...
namespace ISAAR.MSolve.Materials
{
    public class BondSlipCohMat_v2 : ICohesiveZoneMaterial3D_v2 // TODOGerasimos
    {
        ...
        private Matrix ConstitutiveMatrix3D;
        ...
        public void InitializeMatrices()
        {
            ...
            ConstitutiveMatrix3D = Matrix.CreateZero(3, 3);
        ...
        public void UpdateMaterial(double[] epsilon)
        {
            ...
            else
            {
                ...
                while (((Vector.CreateFromArray(res_vec)).Norm2() / t_max) > tol || iter == 0)
                {
                    ...
                    Vector Solution = Vector.CreateZero(5);// = tangent_matrix.SolveLU(res_vec, false);
                    sigma[0] += -Solution[0];
                    sigma[1] += -Solution[1];
                    alpha[0] += -Solution[2];
                    alpha[1] += -Solution[3];
                ...
                Matrix eye5 = Matrix.CreateZero(5, 5); for (int i1 = 0; i1 < 5; i1++) { eye5[i1, i1] = 1; };
                var tangent_matrix_inv = Matrix.CreateZero(5, 5);// tangent_matrix.SolveLU(eye5, false);
