...
{
    public static class HexaBuilder3dok
    {
        public static void MakeHexaBuilding(Model model)
        {
            ...
            for (int l = 0; l < 2; l++)
            {
                for (int k = 0; k < 2; k++)
                {
                    for (int j = 0; j < 2; j++)
                    {
                        model.NodesDictionary.Add(nodeID, new Node() { ID = nodeID, X = startX1 + j * 0.5, Y = startY1 + k * 0.5, Z = startZ1 + l * 1 });
                        nodeID++;
                    }
                }
            }
            ...
            for (int j = 0; j < 2; j++)
            {
                e2.NodesDictionary.Add(4 * (ID2 - 1) + 13, model.NodesDictionary[4 * (ID2 - 1) + 13]); // na allaxthei h arithmisi swsth seira
                e2.NodesDictionary.Add(4 * (ID2 - 1) + 14, model.NodesDictionary[4 * (ID2 - 1) + 14]);
                e2.NodesDictionary.Add(4 * (ID2 - 1) + 16, model.NodesDictionary[4 * (ID2 - 1) + 16]);
                e2.NodesDictionary.Add(4 * (ID2 - 1) + 15, model.NodesDictionary[4 * (ID2 - 1) + 15]);
                ID2++;
            }
            model.ElementsDictionary.Add(e2.ID, e2);
            model.SubdomainsDictionary[subdomainID].ElementsDictionary.Add(e2.ID, e2);
