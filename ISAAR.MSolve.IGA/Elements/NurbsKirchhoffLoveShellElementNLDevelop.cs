    ...
    public class NurbsKirchhoffLoveShellElementNLDevelop : Element, IStructuralIsogeometricElement, ISurfaceLoadedElement
    {
        ...
        public double[] CalculateForcesSlow(IElement element, double[] localDisplacements, double[] localdDisplacements)
        {
            ...
            for (int j = 0; j < gaussPoints.Length; j++)
            {
                ...
                {
                    var contributionsVec = new double[forcesDevelop.Length];
                    ...
                    if (moreChecks)
                    {
                        ...
                        FPKtensorProjected = FPKtensor.ProjectIn3DCartesianBasis();
                    ...
                    for (int i = 0; i < elementControlPoints.Length; i++)
                    {
                        ...
                        for (int r1 = 0; r1 < 3; r1++)
                        {
                            ...
                            var Eab_dr_coeffs = new double[2, 2];
                            bool check1 = true;
                            if (check1)
                            {
                                Eab_dr_coeffs = new double[2, 2]
                                {
                                {dg1_dr.DotProduct(g1)+g1.DotProduct(dg1_dr),dg1_dr.DotProduct(g2)+g1.DotProduct(dg2_dr) },
                                {dg2_dr.DotProduct(g1)+g2.DotProduct(dg1_dr),dg2_dr.DotProduct(g2)+g2.DotProduct(dg2_dr) },
                                ...
                                for (int j1 = 0; j1 < 2; j1++)
                                {
                                    for (int j2 = 0; j2 < 2; j2++)
                                    {
                                        Eab_dr_coeffs[j1, j2] = 0.5 * Eab_dr_coeffs[j1, j2];
                                    }
                                }
                            ...
                            var tensor1 = dF3Dtensor_dr_Tr.SingleContract(defGradTensor).ProjectIn3DCartesianBasis();
                            var tensor2 = defGradTensorTr.SingleContract(dF3Dtensor_dr).ProjectIn3DCartesianBasis();

                            var tensor3 = tensor1.AddTensor(tensor2);
                            var tensor4 = tensor3.Scale(0.5);
                            var dGLtensor_dr = tensor4;
...
#region for higher order terms
                                ...
                                for (int r1 = 0; r1 < 3; r1++)
                                {
                                    ...
                                    for (int s1 = 0; s1 < 3; s1++)
                                    {
                                        ...
                                        if (matrix_vi == 1 | matrix_vi == 2)
                                        {
                                            ...
                                            double[,] Pcontributions = Calculate3DtensorFrom2DcorrectedSlow(FPK_2D, de1_dr, de2_dr, da3_dr, Ei);//revisit this maybe we should pass dg de1_dr instead of dg1_dr.
        ...
        private  double[,] Calculate_FPK_variation_term1_Short(IMatrixView Aijkl_2D, double[] dg1_dr, double[] dg2_dr, double[] de1_dr, double[] de2_dr, double[] G_1, double[] G_2, a3r a3r, double[,] Ei, double[,] ei, double[] g1, double[] g2, double[] dF2D_coefs_dr_vec)//.
        {
            //double[,] dF2D_coefs_dr = new double[2, 2];
            ...
            dF2D_coefs_dr_vec[0] = (dg1_dr[0] * ei[0, 0] + dg1_dr[1] * ei[1, 0] + dg1_dr[2] * ei[2, 0] + g1[0] * de1_dr[0] + g1[1] * de1_dr[1] + g1[2] * de1_dr[2]) * (G_1[0] * Ei[0, 0] + G_1[1] * Ei[1, 0] + G_1[2] * Ei[2, 0]) +
            ...
            dF2D_coefs_dr_vec[2] = (dg1_dr[0] * ei[0, 0] + dg1_dr[1] * ei[1, 0] + dg1_dr[2] * ei[2, 0] + g1[0] * de1_dr[0] + g1[1] * de1_dr[1] + g1[2] * de1_dr[2]) * (G_1[0] * Ei[0, 1] + G_1[1] * Ei[1, 1] + G_1[2] * Ei[2, 1]) +
            ...
            dF2D_coefs_dr_vec[3] = (dg1_dr[0] * ei[0, 1] + dg1_dr[1] * ei[1, 1] + dg1_dr[2] * ei[2, 1]+ g1[0] * de2_dr[0] + g1[1] * de2_dr[1] + g1[2] * de2_dr[2]) * (G_1[0] * Ei[0, 0] + G_1[1] * Ei[1, 0] + G_1[2] * Ei[2, 0]) +
            ...
            dF2D_coefs_dr_vec[1] = (dg1_dr[0] * ei[0, 1] + dg1_dr[1] * ei[1, 1] + dg1_dr[2] * ei[2, 1]+ g1[0] * de2_dr[0] + g1[1] * de2_dr[1] + g1[2] * de2_dr[2]) * (G_1[0] * Ei[0, 1] + G_1[1] * Ei[1, 1] + G_1[2] * Ei[2, 1]) +
        ...
        private double[,] Calculate3DtensorFrom2DcorrectedSlow(double[,] FPK_2D, Vector dg1_dr, Vector dg2_dr, Vector da3_dr, double[,] Ei)
        {
            double[,] eye = new double[3, 3]; eye[0, 0] = 1; eye[1, 1] = 1; eye[2, 2] = 1;
            ...
            double dKsi_r, double dKsi_s, double dHeta_r, double dHeta_s)
        {
            ...
            //5.25
            a3_tilde = CalculateTerm525(surfaceBasisVector3.CopyToArray(), J1, dnorma3_dr, da3tilde_dr, dnorma3_ds, da3tilde_ds);
            ...
            double[] dnorma3_dr, double[][] da3tilde_ds, double[,] dnorma3_drds, double[] a3_tilde, double[,][] da3_drds)
        {
            ...
            for (int r1 = 0; r1 < 3; r1++)
            {
                for (int s1 = 0; s1 < 3; s1++)
                {
                    ...
                    da3_drds[r1, s1] = new double[] // xreiazetai clear edw alla vazoume to new double ok 
                    {
                        firstVec_0 + secondVec_0 + thirdVec_0 + fourthVec_0 + fifthvector_0,
                        firstVec_1 + secondVec_1 + thirdVec_1 + fourthVec_1 + fifthvector_1,
                        firstVec_2 + secondVec_2 + thirdVec_2 + fourthVec_2 + fifthvector_2,
            ...
            double[,][] da3tilde_drds)
        {
            ...
            // xreiazontai clear afta alla afou kanoume new double einai ok 
        ...
        public Tuple<double[], double[]> CalculateStressesDevelop(IElement element, double[] localDisplacements, double[] localdDisplacements)
        {
            ...
            for (int j = 0; j < gaussPoints.Length; j++)
            {
                ...
                for (int i1 = 0; i1 < thicknessPoints.Count; i1++)
                {
                    var materialDevelop = materialsAtThicknessGPDevelop[gaussPoints[j]][thicknessPoints[i1]];
                    var thicknessPoint = thicknessPoints[i1];
        ...
        public double[] CalculateForcesDevelop1(IElement element, double[] localDisplacements, double[] localdDisplacements)
        {
            var shellElement = (NurbsKirchhoffLoveShellElementNLDevelop)element;
            var elementNodalForces = new double[shellElement.ControlPointsDictionary.Count * 3];
            var elementNodalMembraneForces = new double[shellElement.ControlPointsDictionary.Count * 3];
            ...
            for (int j = 0; j < gaussPoints.Length; j++)
            {
                ...
                double[,] jacobian_init = new double[3, 3];
                CalculateJacobian(originalControlPoints, nurbs, j, jacobian_init);
                var a1_init = CalculateSurfaceBasisVector1(jacobian_init, 0);
                var a2_init = CalculateSurfaceBasisVector1(jacobian_init, 1);
                ...
                var a11_init = CalculateSurfaceBasisVector1(originalHessianMatrix, 0);
                var a22_init = CalculateSurfaceBasisVector1(originalHessianMatrix, 1);
                var a12_init = CalculateSurfaceBasisVector1(originalHessianMatrix, 2);
                #endregion
        ...
        public IMatrix StiffnessMatrixDevelop(IElement element)
        {
            var shellElement = (NurbsKirchhoffLoveShellElementNLDevelop)element;
            var gaussPoints = materialsAtThicknessGP.Keys.ToArray();
            var nurbs = CalculateShapeFunctions(shellElement, _controlPoints);
                                ...
                                for (int r1 = 0; r1 < 3; r1++)
                                {
                                    ...
                                    for (int s1 = 0; s1 < 3; s1++)
                                    {
                                        ...
                                        double[,] dFPK2D_coefs_dr = Calculate_FPK_variation_term1_Short(
                                            materialDevelop.ConstitutiveMatrix, dg1_dr, dg2_dr, de1_dr, de2_dr, G_1, G_2, a3r, Ei, ei, g1, g2, dF2D_coefs_dr_vec_prealloc);// to do anti gia copy to array i methodos na mporei na doulevei me IMatrixView
                                        ...
                                        for (int j1 = 0; j1 < 3; j1++)
                                        {
                                            for (int j2 = 0; j2 < 3; j2++)
                                            {
                                                dFPK_tensor_dr_check[j1, j2] += FPK_2D[0, 0] * Ei[j1, 0] * de1_dr[j2] + FPK_2D[0, 1] * Ei[j1, 0] * de2_dr[j2] + FPK_2D[1, 0] * Ei[j1, 1] * de1_dr[j2] + FPK_2D[1, 1] * Ei[j1, 1] * de2_dr[j2] +
                                                    dFPK2D_coefs_dr[0, 0] * Ei[j1, 0] * ei[j2, 0] + dFPK2D_coefs_dr[0, 1] * Ei[j1, 0] * ei[j2, 1] + dFPK2D_coefs_dr[1, 0] * Ei[j1, 1] * ei[j2, 0] + dFPK2D_coefs_dr[1, 1] * Ei[j1, 1] * ei[j2, 1];
                                            }
                                        }
