...
{
    public class Shell8dispCopyGetRAM2 : IStructuralFiniteElement
    {
        ...
        protected Shell8dispCopyGetRAM2()//consztructor apo to hexa8
        {
        }

        public Shell8dispCopyGetRAM2(IFiniteElementMaterial3D material, int gp_d1c, int gp_d2c, int gp_d3c)
        {
            this.gp_d1 = gp_d1c;
            this.gp_d2 = gp_d2c;
            this.gp_d3 = gp_d3c;
        ...
        private byte[,] BL11a_i;
        private byte[,] BL11a_j;
        private bool[,] BL11a_is_not_zero;
        //private double[][,] BL12;
        private byte[,] BL12_i;
        private byte[,] BL12_j;
        private bool[,] BL12_is_not_zero;
        //private double[][,] BNL1;
        private byte[,] BNL1_i;
        private byte[,] BNL1_j;
        private bool[,] BNL1_is_not_zero;
        ...
        private void CalculateInitialConfigurationData(Element element)
        {
            ...
            //    }
            //}
            J_0a_i = new int[3, 16];   // ALLAGH BYTE
            J_0a_j = new int[3, 16];
            for (int k = 0; k < 8; k++)
            {
                //J_0a_i[0, 2 * k] = 0;
                //J_0a_j[0, 2 * k] = Convert.ToByte(3*k);

                //J_0a_i[0, 2 * k + 1] = 0;
                //J_0a_j[0, 2 * k + 1] = Convert.ToByte(3 * k + 1);

                //J_0a_i[1, 2 * k] = 1;
                //J_0a_j[1, 2 * k] = Convert.ToByte(3 * k);

                //J_0a_i[1, 2 * k + 1] = 1;
                //J_0a_j[1, 2 * k + 1] = Convert.ToByte(3 * k + 1);

                //J_0a_i[2, 2 * k] =2;
                //J_0a_j[2, 2 * k] = Convert.ToByte(3 * k);

                //J_0a_i[2, 2 * k + 1] = 2;
                //J_0a_j[2, 2 * k + 1] = Convert.ToByte(3 * k + 1);

                J_0a_i[0, 2 * k] = 0;
                J_0a_j[0, 2 * k] = 3 * k;

                J_0a_i[0, 2 * k + 1] = 0;
                J_0a_j[0, 2 * k + 1] = 3 * k + 1;

                J_0a_i[1, 2 * k] = 1;
                J_0a_j[1, 2 * k] = 3 * k;

                J_0a_i[1, 2 * k + 1] = 1;
                J_0a_j[1, 2 * k + 1] = 3 * k + 1;

                J_0a_i[2, 2 * k] = 2;
                J_0a_j[2, 2 * k] = 3 * k;

                J_0a_i[2, 2 * k + 1] = 2;
                J_0a_j[2, 2 * k + 1] = 3 * k + 1;
            ...
            for (int j = 0; j < nGaussPoints; j++)
            {
                for (int k = 0; k < 3; k++)
                {
                    for (int l = 0; l < 3; l++)
                    {
                        ...
                        for (int m = 0; m < 16; m++)
                        {
                            J_0[j][k, l] += ll1[j][J_0a_i[k, m], J_0a_j[k, m]] * J_0b[m, l];//J_0a[j][k, m] * J_0b[m, l];
                        }
            ...
            BL11a_i = new byte[6, 9];
            BL11a_j = new byte[6, 9];
            BL11a_is_not_zero = new bool[6, 9];
            for (byte k = 0; k < 3; k++)
            {
                for (byte l = 0; l < 3; l++)
                {
                    ...
                    BL11a_i[k, 3 * k + l] = k;
                    BL11a_j[k, 3 * k + l] = l;
                    BL11a_is_not_zero[k, 3 * k + l] = true;
                }
            }
            ...
            for (byte k = 0; k < 2; k++)
            {
                ...
                {
                    //BL11a[j][3 + k, 3 + 3 * k + l] = J_0inv[j][k, l];
                    BL11a_i[3 + k, 3 + 3 * k + l] = k;
                    BL11a_j[3 + k, 3 + 3 * k + l] = l;
                    BL11a_is_not_zero[3 + k, 3 + 3 * k + l] = true;
                }
            }
            ...
            for (byte k = 0; k < 2; k++)
            {
                ...
                {
                    //BL11a[j][3 + k, 3 * k + l] = J_0inv[j][1 + k, l];
                    BL11a_i[3 + k, 3 * k + l] = Convert.ToByte(1 + k);
                    BL11a_j[3 + k, 3 * k + l] = l;
                    BL11a_is_not_zero[3 + k, 3 * k + l] = true;
                }
            }
            ...
            for (byte k = 0; k < 3; k++)
            {
                for (byte l = 0; l < 3; l++)
                {
                    ...
                    BL12_i[3 + k, 3 * k + l] = 1;
                    BL12_j[3 + k, 3 * k + l] = l;
                    BL12_is_not_zero[3 + k, 3 * k + l] = true;
                }
            }
            ...
            for (byte k = 0; k < 3; k++)
            {
                for (byte l = 0; l < 3; l++)
                {
                    ...
                    BL12_i[6 + k, 3 * k + l] = 2;
                    BL12_j[6 + k, 3 * k + l] = l;
                    BL12_is_not_zero[6 + k, 3 * k + l] = true;
                }
            }
            ...
            for (byte m = 0; m < 3; m++)
            {
                for (byte k = 0; k < 3; k++)
                {
                    ...
                    {
                        //BNL1[j][3 * m + k, 3 * m + l] = J_0inv[j][k, l];
                        BNL1_i[3 * m + k, 3 * m + l] = k;
                        BNL1_j[3 * m + k, 3 * m + l] = l;
                        BNL1_is_not_zero[3 * m + k, 3 * m + l] = true;
            ...
            for (int j = 0; j < nGaussPoints; j++)
            {
                ...
                for (int m = 0; m < 8; m++)
                {
                    for (int k = 0; k < 3; k++)
                    {
                        for (int l = 0; l < 3; l++)
                        {
                            BL13[j][3 * k + l, 5 * m + 3] = -ll1[j][J_0a_i[l, m * 2 + 1], J_0a_j[l, m * 2 + 1]] * tUvec[m][3 + k]; //- J_0a[j][l, m * 2 + 1] * tUvec[m][3 + k];
                            BL13[j][3 * k + l, 5 * m + 4] = +ll1[j][J_0a_i[l, m * 2 + 1], J_0a_j[l, m * 2 + 1]] * tUvec[m][k]; //+ J_0a[j][l, m * 2 + 1] * tUvec[m][k];
                        }
                    }
                }
            }
            ...
            for (int j = 0; j < nGaussPoints; j++)
            {
                for (int m = 0; m < 8; m++)
                {
                    for (int k = 0; k < 3; k++)
                    {
                        for (int l = 0; l < 3; l++)
                        {
                            ck[j][m, 3 * k + l] = ll1[j][J_0a_i[l, 2 * m + 1], J_0a_j[l, 2 * m + 1]]* tU[m][3 + k];//J_0a[j][l, 2 * m + 1] * tU[m][3 + k];
                        }
                    }
                }
            }
            ...
            for (int j = 0; j < nGaussPoints; j++)
            {
                for (int k = 0; k < 3; k++)
                {
                    for (int l = 0; l < 3; l++)
                    {
                        ...
                        for (int m = 0; m < 16; m++)
                        {
                            J_1[j][k, l] += ll1[j][J_0a_i[k, m], J_0a_j[k, m]] * J_1b[m, l];//J_0a[j][k, m] * J_1b[m, l];
                        }
            ...
            for (int j = 0; j < nGaussPoints; j++)
            {
                for (int k = 0; k < 6; k++)
                {
                    for (int l = 0; l < 9; l++)
                    {
                        ...
                        for (int m = 0; m < 9; m++)
                        {
                            BL11[j][k, l] += (BL11a_is_not_zero[k, m] ? J_0inv[j][BL11a_i[k, m], BL11a_j[k, m]] * BL11b[j][m, l] : 0);    //BL11a[j][k, m] * BL11b[j][m, l];
                        }
                    }
                }
            }
            ...
            for (int j = 0; j < nGaussPoints; j++)
            {
                for (int k = 0; k < 6; k++)
                {
                    for (int l = 0; l < 9; l++)
                    {
                        ...
                        for (int m = 0; m < 9; m++)
                        {
                            BL1_2[j][k, l] += (BL12_is_not_zero[m, l] ? J_0inv[j][BL12_i[m, l], BL12_j[m, l]] * BL11[j][k, m] : 0);    // BL11[j][k, m] * BL12[j][m, l]; //TODO BL11 keno kai BL12 null thelei getbl12 edw kai parakatw
                ...
                for (int k = 0; k < 6; k++)
                {
                    for (int l = 0; l < 9; l++)
                    {
                        BL01plus1_2[j][k, l] = (BL11a_is_not_zero[k, l] ? J_0inv[j][BL11a_i[k, l], BL11a_j[k, l]] + BL1_2[j][k, l] : BL1_2[j][k, l]);    //BL1_2[j][k, l] + BL11a[j][k, l];
                    }
                }
            ...
            for (int j = 0; j < nGaussPoints; j++)
            {
                ...
                for (int k = 0; k < 9; k++)
                {
                    for (int l = 0; l < 40; l++)
                    {
                        ...
                        for (int m = 0; m < 9; m++)
                        {
                            BNL[k, l] += (BNL1_is_not_zero[k, m] ? J_0inv[j][BNL1_i[k, m], BNL1_j[k, m]] * BL13[j][m, l] : 0);    //BNL1[j][k, m] * BL13[j][m, l];
                        }
