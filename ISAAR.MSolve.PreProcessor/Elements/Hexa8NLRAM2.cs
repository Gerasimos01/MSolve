...
{
    public class Hexa8NLRAM2 : IStructuralFiniteElement , IEmbeddedHostElement
    {
        ...
        protected Hexa8NLRAM2()//consztructor apo to hexa8
        {
        }

        public Hexa8NLRAM2(IFiniteElementMaterial3D material, int gp_d1c, int gp_d2c, int gp_d3c)
        {
        ...
        private byte[,] BL13_hexa_i;
        private byte[,] BL13_hexa_j;
        private bool[,] BL13_hexa_is_not_zero;
        ...
        private byte[,] BL11a_hexa_i;
        private byte[,] BL11a_hexa_j;
        private bool[,] BL11a_hexa_is_not_zero;
        //private double[][,] BL12_hexa;
        private byte[,] BL12_hexa_i;
        private byte[,] BL12_hexa_j;
        private bool[,] BL12_hexa_is_not_zero;
        //private double[][,] BL01_hexa;
        private byte[,] BL01_hexa_i;
        private byte[,] BL01_hexa_j;
        private bool[,] BL01_hexa_is_not_zero;
        ...
        private void CalculateInitialConfigurationData(Element element)
        {
            ...
            BL13_hexa_i= new byte[9, 24];
            BL13_hexa_j = new byte[9, 24];
            BL13_hexa_is_not_zero = new bool[9, 24];
            for (byte m = 0; m < 8; m++)
            {
                for (byte n = 0; n < 3; n++)
                {
                    BL13_hexa_i[n, 3 * m + 0] = n;
                    BL13_hexa_i[n + 3, 3 * m + 1] = n;
                    BL13_hexa_i[n + 6, 3 * m + 2] = n;
                    BL13_hexa_j[n, 3 * m + 0] = m;
                    BL13_hexa_j[n + 3, 3 * m + 1] = m;
                    BL13_hexa_j[n + 6, 3 * m + 2] = m;
                    BL13_hexa_is_not_zero[n, 3 * m + 0] = true;
                    BL13_hexa_is_not_zero[n + 3, 3 * m + 1] = true;
                    BL13_hexa_is_not_zero[n + 6, 3 * m + 2] = true;
                }
            }
            ...
            BL11a_hexa_i = new byte[6, 9];
            BL11a_hexa_j = new byte[6, 9];
            BL11a_hexa_is_not_zero = new bool[6, 9];
            for (byte m = 0; m < 3; m++) // upologismos triwn prwtwn grammwn
            {
                for (byte n = 0; n < 3; n++)
                {
                    BL11a_hexa_i[m, 3 * m + n] = m;
                    BL11a_hexa_j[m, 3 * m + n] = n;
                    BL11a_hexa_is_not_zero[m, 3 * m + n] = true;
                }
            }
            for (byte n = 0; n < 3; n++)
            {
                BL11a_hexa_i[3, n] = 1; // upologismos 4hs gramms
                BL11a_hexa_j[3, n] = n;
                BL11a_hexa_is_not_zero[3, n] = true;

                BL11a_hexa_i[3, 3 + n] = 0;
                BL11a_hexa_j[3, 3 + n] = n;
                BL11a_hexa_is_not_zero[3, 3 + n] = true;

                BL11a_hexa_i[4, 3 + n] = 2; // upologismos 5hs gramms
                BL11a_hexa_j[4, 3 + n] = n;
                BL11a_hexa_is_not_zero[4, 3 + n] = true;

                BL11a_hexa_i[4, 6 + n] = 1;
                BL11a_hexa_j[4, 6 + n] = n;
                BL11a_hexa_is_not_zero[4, 6 + n] = true;

                BL11a_hexa_i[5, 0 + n] = 2; // upologismos 6hs gramms
                BL11a_hexa_j[5, 0 + n] = n;
                BL11a_hexa_is_not_zero[5, 0 + n] = true;

                BL11a_hexa_i[5, 6 + n] = 0;
                BL11a_hexa_j[5, 6 + n] = n;
                BL11a_hexa_is_not_zero[5, 6 + n] = true;
            }
            BL12_hexa_i = new byte[9, 9];
            BL12_hexa_j = new byte[9, 9];
            BL12_hexa_is_not_zero = new bool[9, 9];
            for (byte m = 0; m < 3; m++) // upologismos triwn prwtwn grammwn
            {
                for (byte n = 0; n < 3; n++)
                {
                    BL12_hexa_i[m, 3 * m + n] = 0;
                    BL12_hexa_j[m, 3 * m + n] = n;
                    BL12_hexa_is_not_zero[m, 3 * m + n] = true;
                }
            }
            for (byte m = 0; m < 3; m++) // upologismos grammwn 4 ews 6
            {
                for (byte n = 0; n < 3; n++)
                {
                    BL12_hexa_i[3 + m, 3 * m + n] = 1;
                    BL12_hexa_j[3 + m, 3 * m + n] = n;
                    BL12_hexa_is_not_zero[3 + m, 3 * m + n] = true;
                }
            }
            for (byte m = 0; m < 3; m++) // upologismos grammwn 7 ews 8
            {
                for (byte n = 0; n < 3; n++)
                {
                    BL12_hexa_i[6 + m, 3 * m + n] = 2;
                    BL12_hexa_j[6 + m, 3 * m + n] = n;
                    BL12_hexa_is_not_zero[6 + m, 3 * m + n] = true;
                }
            }
            BL01_hexa_i = new byte[6, 9];
            BL01_hexa_j = new byte[6, 9];
            BL01_hexa_is_not_zero = new bool[6, 9];
            for (byte m = 0; m < 3; m++) // upologismos triwn prwtwn grammwn
            {
                for (byte n = 0; n < 3; n++)
                {
                    BL01_hexa_i[m, 3 * m + n] = m;
                    BL01_hexa_j[m, 3 * m + n] = n;
                    BL01_hexa_is_not_zero[m, 3 * m + n] = true;
                }
            }
            for (byte n = 0; n < 3; n++)
            {
                BL01_hexa_i[3, n] = 1; // upologismos 4hs gramms
                BL01_hexa_j[3, n] = n;
                BL01_hexa_is_not_zero[3, n] = true;

                BL01_hexa_i[3, 3 + n] = 0;
                BL01_hexa_j[3, 3 + n] = n;
                BL01_hexa_is_not_zero[3, 3 + n] = true;

                BL01_hexa_i[4, 3 + n] = 2; // upologismos 5hs gramms
                BL01_hexa_j[4, 3 + n] = n;
                BL01_hexa_is_not_zero[4, 3 + n] = true;

                BL01_hexa_i[4, 6 + n] = 1;
                BL01_hexa_j[4, 6 + n] = n;
                BL01_hexa_is_not_zero[4, 6 + n] = true;

                BL01_hexa_i[5, 0 + n] = 2; // upologismos 6hs gramms
                BL01_hexa_j[5, 0 + n] = n;
                BL01_hexa_is_not_zero[5, 0 + n] = true;

                BL01_hexa_i[5, 6 + n] = 0;
                BL01_hexa_j[5, 6 + n] = n;
                BL01_hexa_is_not_zero[5, 6 + n] = true;
            }
            for (int gpoint = 0; gpoint < nGaussPoints; gpoint++)
            {
                ...
                for (int m = 0; m < 9; m++)
                {
                    for (int n = 0; n < 24; n++)
                    {
                        ...
                        for (int p = 0; p < 9; p++)
                        {
                            BNL_hexa[gpoint][m, n] += (BL13_hexa_is_not_zero[p, n] ? BNL1_hexa[gpoint][m, p] * ll1_hexa[gpoint][BL13_hexa_i[p, n], BL13_hexa_j[p, n]] : 0);    //BNL1_hexa[gpoint][m, p] * BL13_hexa[gpoint][p, n];
                        }
                    }
                }
            }
            ...
            for (int npoint = 0; npoint < nGaussPoints; npoint++)
            {
                ...
                for (int m = 0; m < 6; m++)
                {
                    for (int n = 0; n < 3; n++)
                    {
                        for (int p = 0; p < 3; p++)
                        {
                            BL11[npoint][m, n] += (BL11a_hexa_is_not_zero[m, p] ?J_0inv_hexa[npoint][BL11a_hexa_i[m, p], BL11a_hexa_j[m, p]]*l_perisp[p, n] : 0);    // BL11a_hexa[npoint][m, p] * l_perisp[p, n];
                            BL11[npoint][m,3+n]+= (BL11a_hexa_is_not_zero[m, 3 + p] ? J_0inv_hexa[npoint][BL11a_hexa_i[m, 3 + p], BL11a_hexa_j[m, 3 + p]] * l_perisp[p, n] : 0);    //BL11a_hexa[npoint][m, 3+p] * l_perisp[p, n];
                            BL11[npoint][m, 6 + n] += (BL11a_hexa_is_not_zero[m, 6 + p] ? J_0inv_hexa[npoint][BL11a_hexa_i[m, 6 + p], BL11a_hexa_j[m, 6 + p]] * l_perisp[p, n] : 0);    // BL11a_hexa[npoint][m, 6 + p] * l_perisp[p, n];
                        }
                    }
                }
                ...
                for (int m = 0; m < 6; m++)
                {
                    for (int n = 0; n < 9; n++)
                    {
                        ...
                        for (int p = 0; p < 9; p++)
                        {
                            BL1112sun01_hexa[npoint][m, n] += (BL12_hexa_is_not_zero[p, n] ? J_0inv_hexa[npoint][BL12_hexa_i[p, n], BL12_hexa_j[p, n]] * BL11[npoint][m, p] : 0);    // BL11[npoint][m, p] * BL12_hexa[npoint][p, n];
                        }
                    }
                }
                for (int m = 0; m < 6; m++)
                {
                    for (int n = 0; n < 9; n++)
                    {
                        BL1112sun01_hexa[npoint][m, n] += (BL01_hexa_is_not_zero[m, n] ? J_0inv_hexa[npoint][BL01_hexa_i[m, n], BL01_hexa_j[m, n]]: 0);    //  BL01_hexa[npoint][m, n];
                    }
                }
                ...
                for (int m = 0; m < 6; m++)
                {
                    for (int n = 0; n < 24; n++)
                    {
                        ...
                        for (int p = 0; p < 9; p++)
                        {
                            BL[npoint][m, n] += (BL13_hexa_is_not_zero[p, n] ? ll1_hexa[npoint][BL13_hexa_i[p, n], BL13_hexa_j[p, n]] * BL1112sun01_hexa[npoint][m, p] : 0);    //BL1112sun01_hexa[npoint][m, p] * BL13_hexa[npoint][p, n];
                        }
                    }
                }
        ...
        public virtual IMatrix2D<double> StiffnessMatrix(Element element)
        {
            ...
            k_stoixeiou=this.UpdateKmatrices();
