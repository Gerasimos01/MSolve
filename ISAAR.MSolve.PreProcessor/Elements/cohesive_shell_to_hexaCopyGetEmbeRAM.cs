...
{
    public class cohesive_shell_to_hexaCopyGetEmbeRAM : IStructuralFiniteElement, IEmbeddedElement
    {
        ...
        protected cohesive_shell_to_hexaCopyGetEmbeRAM()//consztructor apo to hexa8
        {
        }

        public cohesive_shell_to_hexaCopyGetEmbeRAM(IFiniteElementMaterial3D material, int gp_d1c, int gp_d2c)
        {
        ...
        private double [][] UpdateCoordinateData(double[] localdisplacements) // sto shell8disp sto calculate forces kaleitai me this.UpdateCoordinateData(localTotalDisplacements);
        {
            ...
            ////PrintUtilities.ConvertAndWriteToFileVector(Delta, @"C:\Users\turbo-x\Desktop\cohesive_check_MSOLVE_2\paradeigma_apo_arxika_swsta_shell_orthi_gia_check_tou_neou_class\orthi\CopyApoTaShellNewLoadCaseArgurhs\unused_anest_kai_t\Delta_output.txt");
            this.UpdateTMatrix();
            print_counter += 1;
            //if (print_counter == 1)
            //{
            //    PrintUtilities.ConvertAndWriteToFileVector(ox_i, @"C:\Users\turbo-x\Desktop\cohesive_check_MSOLVE_2\paradeigma_apo_arxika_swsta_shell_orthi_gia_check_tou_neou_class\orthi\CopyApoTaShellNewLoadCaseArgurhs\unused_anest_kai_t\ox_i_anestra.txt");
            //    PrintUtilities.ConvertAndWriteToFileVector(ox_i_shell_midsurface, @"C:\Users\turbo-x\Desktop\cohesive_check_MSOLVE_2\paradeigma_apo_arxika_swsta_shell_orthi_gia_check_tou_neou_class\orthi\CopyApoTaShellNewLoadCaseArgurhs\unused_anest_kai_t\ox_i_shell_midsurface_anestra.txt");
        ...
        private double [,] UpdateKmatrices()
        {
            ...
            for (int npoint1 = 0; npoint1 < nGaussPoints; npoint1++)
            {
                ...
                //    }
                //}
                double [,] D_RN3_sunt_ol = new double[3, 24];
                for (int l = 0; l < 3; l++)
                {
                    for (int m = 0; m < 24; m++)
                    {
        ...
        public Tuple<double[], double[]> CalculateStresses(Element element, double[] localTotalDisplacementsSuperElement, double[] localdDisplacementsSuperElement)
        {
            ...
            Delta=this.UpdateCoordinateData(localTotalDisplacements);
        ...
        public virtual IMatrix2D<double> StiffnessMatrix(Element element)
        {
            ...
            if (N1 == null)
            {
                ...
                this.UpdateCoordinateData(new double[64]); // mporei na lamvanetai apo to update... to return tou (pou einai Delta[][] p.x. gia material)
                this.InitializeRN3();
            }
            //for (int i = 0; i < materialsAtGaussPoints.Length; i++)
            //{
            ...
            IMatrix2D<double> superElement_stiffnessMatrix= dofEnumerator.GetTransformedMatrix(element_stiffnessMatrix);// PROSTHIKI GIA CHECK EMBEDDDED
