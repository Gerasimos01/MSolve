...
namespace ISAAR.MSolve.FEM.Entities
{
    public class Subdomain_v2 : ISubdomain_v2
    {
            ...
            int nIncrement, int totalIncrements)
        {
            ...
            foreach (Element element in Elements)
            {
                var localSolution = GetLocalVectorFromGlobalWithoutPrescribedDisplacements(element, solution).ToRawArray();
                ...
                var localdSolution = GetLocalVectorFromGlobalWithoutPrescribedDisplacements(element, dSolution).ToRawArray();
