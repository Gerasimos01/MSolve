...
{
    public class Hexa8NLRAM_1copyMSPrintStrainHist : IStructuralFiniteElement , IEmbeddedHostElement
    {
        ...
        protected Hexa8NLRAM_1copyMSPrintStrainHist()//consztructor apo to hexa8
        {
        }

        public Hexa8NLRAM_1copyMSPrintStrainHist(IFiniteElementMaterial3D material, int gp_d1c, int gp_d2c, int gp_d3c)
        {
        ...
        string string1 = @"C:\Users\Dimitris Giovanis\Desktop\notes_elegxoi\MSOLVE_output_2\DefGradVecElement{0}GP{1}Iter{2}.txt";
        string string2 = @"C:\Users\Dimitris Giovanis\Desktop\notes_elegxoi\MSOLVE_output_2\StressVecElement{0}GP{1}Iter{2}.txt";
        string string3 = @"C:\Users\Dimitris Giovanis\Desktop\notes_elegxoi\MSOLVE_output_2\localTotaldisplaElement{0}Iter{1}.txt";
        string string4 = @"C:\Users\Dimitris Giovanis\Desktop\notes_elegxoi\MSOLVE_output_2\ConsElement{0}GP{1}Iter{2}.txt";
        public Tuple<double[], double[]> CalculateStresses(Element element, double[] localTotalDisplacements, double[] localdDisplacements)
        {
            ...
            string n_element_data = element.ID.ToString();
            ...
            for (int npoint = 0; npoint < materialsAtGaussPoints.Length; npoint++)
            {
                ...
                // prosthiki print
                string n_GP_data = npoint.ToString();
                string path1 = string.Format(string1,n_element_data,  n_GP_data, counter_data);
                string path2 = string.Format(string2,n_element_data,  n_GP_data, counter_data);
                string path4 = string.Format(string4, n_element_data, n_GP_data, counter_data);
                //solution.WriteToFile(path);                
                WriteToFileVector(DefGradVec[npoint], path1);
                WriteToFileVector(materialsAtGaussPoints[npoint].Stresses, path2);
                double[,] consData = new double[6, 6];
                for (int i1 = 0; i1 < 6; i1++)
                {for (int i2 = 0; i2 < 6; i2++) { consData[i1, i2] = materialsAtGaussPoints[npoint].ConstitutiveMatrix[i1, i2]; }                 }

                PrintUtilities.WriteToFile(consData,path4);
            }

            //prosthiki print 
            string path3 = string.Format(string3, n_element_data, counter_data);
            WriteToFileVector(localTotalDisplacements, path3);
